            ...
            <div
              className={`h-2 flex-1 rounded-full transition-colors ${
                ...
                (['config', 'sections', 'generating', 'preview'].indexOf(s) < ['config', 'sections', 'generating', 'preview'].indexOf(step))
                  ? 'w-12 bg-indigo-200' : 'w-12 bg-gray-200'
              }`}
            />
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">
              Document Title <span className="text-xs font-normal text-gray-400">(optional)</span>
            </label>
            <input
              type="text"
              value={title}
          ...
          <div className="flex flex-col gap-1.5 max-h-60 overflow-y-auto">
            {sections.map((sec, idx) => (
              ...
              >
                ...
                <span className="flex-1 font-medium text-gray-700">{sec}</span>
                <button
                  type="button"
      ...
      {step === 'generating' && (
        <div className="flex flex-col items-center gap-4 py-12">
          <div className="relative">
            <Doki size={64} className="animate-pulse" />
            <Loader2 className="absolute -bottom-1 -right-1 h-6 w-6 text-indigo-500 animate-spin" />
          </div>
          <div className="text-center">
            <p className="font-semibold text-gray-800">Doki is writing your document…</p>
            ...
            <div className="flex items-center gap-2">
              {clauseRefs.length > 0 && (
                <div className="flex items-center gap-1">
                  ...
                  {clauseRefs.length > 5 && (
                    <span className="text-xs text-gray-400">+{clauseRefs.length - 5}</span>
                  )}
                </div>
              )}
            </div>
