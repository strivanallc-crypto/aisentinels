    ...
    <div className="p-6 max-w-[1280px]">
              ...
              <div key={r.id} className="flex items-center gap-4 px-4 py-4 transition-all duration-200 hover:bg-white/[0.03] hover:pl-5 group">
                ...
                {r.legalHold ? (
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button onClick={() => handleReleaseLegalHold(r.id)} className="rounded p-0.5 hover:bg-white/10" title="Release hold" style={{ color: 'var(--muted)' }}>
                      <Unlock className="h-3 w-3" />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => { setHoldingId(r.id); setShowHold(true); }} className="flex items-center gap-1 text-[11px] rounded-full px-2 py-0.5 hover:bg-white/5" style={{ color: 'var(--muted)' }}>
                    <Lock className="h-3 w-3" /> Hold
                  </button>
                )}
      ...
      <Modal open={showCreate} onOpenChange={(o) => { setShowCreate(o); if (!o) setForm(EMPTY_FORM); }} title="New Record">
        <form onSubmit={handleCreate} className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" required value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="e.g. Supplier Qualification Record Q-2024-001" className="w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: 'var(--border)', background: 'var(--input-bg)', color: 'var(--text)' }} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Category</label>
              <select required value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as RecordCategory }))} className="w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: 'var(--border)', background: 'var(--input-bg)', color: 'var(--text)' }}>
                {CATEGORIES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Retention (years)</label>
              <input type="number" min={1} max={99} value={form.retentionYears} onChange={(e) => setForm((f) => ({ ...f, retentionYears: Number(e.target.value) }))} className="w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: 'var(--border)', background: 'var(--input-bg)', color: 'var(--text)' }} />
            </div>
          </div>
          <div>
            ...
            <textarea rows={4} value={form.contentText} onChange={(e) => setForm((f) => ({ ...f, contentText: e.target.value }))} placeholder="Record content..." className="w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: 'var(--border)', background: 'var(--input-bg)', color: 'var(--text)' }} />
          </div>
      ...
      <Modal open={showHold} onOpenChange={(o) => { setShowHold(o); if (!o) { setHoldingId(null); setHoldReason(''); } }} title="Apply Legal Hold">
        <div className="flex flex-col gap-4">
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>A legal hold prevents this record from being modified or deleted until released.</p>
          <div>
            <label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Reason</label>
            <textarea rows={2} value={holdReason} onChange={(e) => setHoldReason(e.target.value)} placeholder="Reason for legal hold..." className="w-full rounded-lg border px-3 py-2 text-sm outline-none" style={{ borderColor: 'var(--border)', background: 'var(--input-bg)', color: 'var(--text)' }} />
          </div>
